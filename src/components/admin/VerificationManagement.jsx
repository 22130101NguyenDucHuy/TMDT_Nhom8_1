...
export default function VerificationManagement() {
  ...
  const loadRequests = async (background) => {
  ...
  const handleApprove = async (id, userId) => {
    if (!window.confirm("Bạn có chắc chắn muốn duyệt thẻ sinh viên này?")) return;
    setProcessingId(id);
    try {
  ...
  const handleReject = async (id, userId) => {
    if (!window.confirm("Bạn có chắc chắn muốn từ chối thẻ sinh viên này?")) return;
    setProcessingId(id);
    try {
