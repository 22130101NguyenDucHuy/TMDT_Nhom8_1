...
export default function AuthModal() {
  const { isAuthModalOpen, authModalMode, setAuthModalMode, closeAuthModal, showToast, signInWithOAuth } = useAuth();
    ...
    try {
      if (authModalMode === "login") {
        ...
        if (data.user) {
          ...
          if (userError) console.error("Lỗi khi lưu vào bảng lb_users:", userError);
        }
  ...
  const handleSubmitVerification = async () => {
    ...
    try {
      ...
      const { error: dbError } = await supabase
        ...
        .insert([{ user_id: registeredUserId, image_path: filePath, status: 'pending' }]);
      if (dbError) throw dbError;

      showToast("Đã gửi ảnh xác thực! Vui lòng đợi admin duyệt.", "success");
      closeAuthModal();
    } catch (err) {
  ...
  const isRegistering = authModalMode === "register";
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 pt-2 border-b border-slate-100">
          <div className="flex gap-6">
            <button type="button" disabled={verificationStep}
              onClick={() => { if (verificationStep) return; resetForm(); setAuthModalMode("register"); }}
              className={`py-3 text-lg font-bold border-b-2 transition-colors ${verificationStep ? 'text-slate-300 cursor-not-allowed' : isRegistering ? "text-teal-700 border-teal-700" : "text-slate-400 border-transparent hover:text-slate-600"}`}>
              Đăng ký
            </button>
            <button type="button" disabled={verificationStep}
              onClick={() => { if (verificationStep) return; resetForm(); setAuthModalMode("login"); }}
              className={`py-3 text-lg font-bold border-b-2 transition-colors ${verificationStep ? 'text-slate-300 cursor-not-allowed' : !isRegistering ? "text-teal-700 border-teal-700" : "text-slate-400 border-transparent hover:text-slate-600"}`}>
              Đăng nhập
            </button>
          </div>
          <button onClick={verificationStep ? undefined : closeAuthModal}
        ...
        <div className="p-6">
          ...
          {verificationStep && (
            <div>
              ...
              <div className="border-2 border-dashed border-slate-300 rounded-lg p-8 text-center cursor-pointer hover:border-teal-500"
              ...
              <button onClick={handleSubmitVerification} disabled={verificationLoading || !verificationFile}
              ...
              <button onClick={switchToLogin}
                className="w-full mt-2 text-sm text-slate-500 hover:text-teal-700 transition-colors underline underline-offset-2">
            ...
            <form onSubmit={handleAuth} className="space-y-4">
              <div>
                ...
                <input type="email" required
                  placeholder={isRegistering ? "email@student.hcmut.edu.vn" : "hello@loopbook.vn"}
                  className="vinted-input"
                  value={email} onChange={(e) => setEmail(e.target.value)} />
                {isRegistering && (
                  <p className="text-xs text-slate-400 mt-1">Chỉ email sinh viên @...edu.vn mới được đăng ký</p>
                )}
              </div>
              ...
              <button type="submit" disabled={loading || (isRegistering && password !== confirmPassword)}
              ...
              {isRegistering && (
                <div className="space-y-3">
                  ...
                  <button type="button" onClick={() => signInWithOAuth('azure')}
                    className="w-full flex items-center justify-center gap-3 px-4 py-2.5 border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-all">
