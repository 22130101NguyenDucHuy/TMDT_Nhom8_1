...
export default function BookDetailScreen() {
      ...
      if (bookData) {
        ...
        if (relData) {
          setRelated(relData.map((b) => {
            const relImgs = resolveBookImages(b.id, b.images);
            return {
              ...b,
              images: relImgs,
  ...
  const handleSubmitReport = async () => {
    ...
    try {
      const { error } = await supabase.from("lb_reports").insert([{
      ...
      if (error) {
        if (error.code === "23505") {
          showToast("Bạn đã báo cáo sản phẩm này rồi", "error");
  ...
  if (book.status === 'suspended') {
    return (
      <div className="max-w-2xl mx-auto py-16 text-center">
        ...
        <h2 className="text-xl font-bold text-slate-800 mb-2">Tài liệu không khả dụng</h2>
        <p className="text-slate-500 text-sm mb-5 max-w-md mx-auto">Tài liệu này hiện không khả dụng do tài khoản người bán đã bị khóa.</p>
    ...
    <div className="max-w-6xl mx-auto py-4 md:py-6">
      ...
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
        ...
        <div className="w-full lg:w-[45%] flex flex-col">
          ...
          {book.status === "active" && !book.is_sold && (
          ...
          <div className="flex flex-col gap-2.5 mb-5">
            <button onClick={handleOpenReport} className="text-xs text-slate-400 hover:text-red-500 transition-colors flex items-center justify-center gap-1 py-1">
      ...
      {showReportModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => { if (!submittingReport) setShowReportModal(false); }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6 relative" onClick={(e) => e.stopPropagation()}>
            ...
            {checkingReport ? (
              <div className="py-8 text-center text-slate-500 text-sm">Đang kiểm tra...</div>
            ) : alreadyReported ? (
              <div className="py-6 text-center">
                ...
                <p className="text-slate-800 font-semibold mb-1">Bạn đã báo cáo sản phẩm này</p>
                <p className="text-slate-500 text-sm">Cảm ơn bạn đã góp phần giữ cộng đồng an toàn. Chúng tôi sẽ xem xét trong 24h.</p>
                <button onClick={() => setShowReportModal(false)}
                  className="mt-5 px-6 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-lg text-sm transition-colors">
                  Đóng
                </button>
              </div>
            ) : (
